...
def save_results_with_pvalues(results, config, p_values, stats_results, output_dir):
    ...
    for model_key, model_results in results.items():
        for key, value in model_results.items():
            # Ensure key is a tuple of (train_key, test_key)
            if isinstance(key, tuple) and len(key) == 2:
                train_key, test_key = key
            else:
                raise ValueError(f"Unexpected key format: {key}. Expected a tuple of (train_key, test_key).")

            # Format key and store results
            formatted_key = f"{model_key}_{train_key}_train_{test_key}_test"
...
def main():
    ...
    # Separate output directories for Random Forest and 1NN
    rf_output_dir = os.path.join(base_output_dir, "random_forest")
    nn_output_dir = os.path.join(base_output_dir, "1nn")
    ...
    results = {"random_forest": {}, "1nn": {}}  # Ensure proper initialization
    p_values = []
    ...
    for train_key, train_data in datasets.items():
        ...
        for test_key, test_data in datasets.items():
            ...
            rf_mse, rf_r2, rf_delta_z, rf_var, rf_skew, rf_kurto = evaluate_model(
                y_test, y_pred_rf, test_data, nside, rf_output_dir, train_key, test_key
            )
            ...
            nn_mse, nn_r2, nn_delta_z, nn_var, nn_skew, nn_kurto = evaluate_model(
                y_test, y_pred_nn, test_data, nside, nn_output_dir, train_key, test_key
            )
            results["1nn"][(train_key, test_key)] = {
        ...
        #print(f"Results after processing {train_key}: {results}")

    # Perform statistical tests
    model_keys = ["random_forest", "1nn"]
    for model_key in model_keys:
        print(f"Performing statistical tests for {model_key}")
        keys = list(results[model_key].keys())
        print(f"Keys for {model_key}: {keys}")
        for i, key1 in enumerate(keys):
            for key2 in keys[i + 1:]:
                try:
                    delta_z1 = np.array(results[model_key][key1]["Delta_z"])
                    delta_z2 = np.array(results[model_key][key2]["Delta_z"])
                except KeyError as e:
                    print(f"KeyError: {e}. Check key formatting in results.")
                    continue
                ...
                p_values.append({
                    ...
                    "students_t_test": p_value_students,
                    "welchs_t_test": p_value_welch,
                    "variance_test": p_value_var
                })
