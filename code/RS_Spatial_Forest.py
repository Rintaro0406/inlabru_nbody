...
def check_memory(threshold=0.8):
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            print(f"[{func.__name__}] Memory usage before execution: {memory_before:.2f} MB")
            result = func(*args, **kwargs)
            memory_after = process.memory_info().rss / (1024**2)  # MB
            print(f"[{func.__name__}] Memory usage after execution: {memory_after:.2f} MB")
            gc.collect()
            return result
        return wrapper
    return decorator
...
def load_datasets(input_paths):
    return {key: pd.read_csv(path, compression='bz2') for key, path in input_paths.items()}
...
@measure_time
def main():
    config = read_yaml_config("/Users/r.kanaki/code/inlabru_nbody/config/RS_Spatial_Forest.yml")
    input_paths = config["input_paths"]
    output_dir = config["output_paths"]["base_dir"]
    os.makedirs(output_dir, exist_ok=True)

    datasets = load_datasets(input_paths)
    features = ["des_asahi_full_g_true", "des_asahi_full_r_true",
    ...
    for train_key, train_data in datasets.items():
        ...
        plot_feature_importance(rf.feature_importances_, features, output_dir, train_key)
        ...
        for test_key, test_data in datasets.items():
            ...
            mse, r2, delta_z = evaluate_model(y_test, y_pred, test_data, nside,
                                              output_dir, train_key, test_key)
