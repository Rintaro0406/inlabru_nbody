...
def plot_heatmap(matrix, title, output_path, cmap="coolwarm", fmt=".4g", vmin=None, vmax=None, annot=True):
    if not isinstance(matrix, (pd.DataFrame, np.ndarray)):
        raise ValueError(f"Expected 2D matrix, but got {type(matrix)} with value {matrix}")
    if isinstance(matrix, np.ndarray) and matrix.ndim != 2:
        raise ValueError(f"Expected 2D numpy array, but got shape {matrix.shape}")
...
def create_performance_matrices(results):
    ...
    for key, value in results.items():
        try:
            # Match the model name (random_forest or 1nn) and split the key
            ...
            # Extract train, test, and mode from the key
            parts = rest.split("_train_")
            if len(parts) != 2 or "_test_" not in parts[1]:
                print(f"Skipping key '{key}': Unexpected format.")
                continue

            train = parts[0].lower()
            test, mode = parts[1].split("_test_")
            test = test.lower()
            mode = mode.lower()
...
def main(config_path):
    ...
    for model in performance_matrices:
        for mode in performance_matrices[model]:
            for metric, matrix in performance_matrices[model][mode].items():
                if isinstance(matrix, pd.DataFrame):
                    title = f"{model.upper()} {mode.capitalize()} {metric.replace('_', ' ').title()} Heatmap"
                    output_path = os.path.join(output_dir, f"{model}_{mode}_{metric}_heatmap.png")
...
if __name__ == "__main__":
    main("/Users/r.kanaki/code/inlabru_nbody/config/RS_heatmap_micat1_schnell.yml")