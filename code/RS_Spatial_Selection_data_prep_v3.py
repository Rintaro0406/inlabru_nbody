...
def stratify_data(data, healpix_map, nside):
    ...
    # Add pixel indices to the DataFrame for filtering
    ...
    filtered_healpix_map_valid = filtered_healpix_map[filtered_healpix_map > 7000]

    # Determine density threshold for high-density pixels
    threshold = np.percentile(filtered_healpix_map_valid, 90)
    ...
    # Filter data into high-density and low-density subsets based on pixel indices
    high_density_data = data_filtered[data_filtered["pixel_index"].isin(high_density_pixels)]
...
def plot_color_color_diagram(data, output_path, subset_name="all"):
    ...
    data["g-r"] = data["des_asahi_full_g_true"] - data["des_asahi_full_r_true"]
    data["r-i"] = data["des_asahi_full_r_true"] - data["des_asahi_full_i_true"]
    data["i-z"] = data["des_asahi_full_i_true"] - data["des_asahi_full_z_true"]
    data["z-y"] = data["des_asahi_full_z_true"] - data["des_asahi_full_y_true"]
    data["y-g"] = data["des_asahi_full_y_true"] - data["des_asahi_full_g_true"]

    # Define color pairs to plot
    color_pairs = [("g-r", "r-i"), ("r-i", "i-z"), ("i-z", "z-y"), ("y-g", "g-r")]
    ...
    for x_color, y_color in color_pairs:
        ...
        scatter = plt.scatter(
            x=data[x_color],
            y=data[y_color],
            c=data["z_cgal"],  # Use true redshift for color mapping
            cmap="plasma",
            s=1,  # Point size
            alpha=0.7
        )
        ...
        plt.title(f"Color-Color Diagram ({x_color} vs {y_color}) - {subset_name.capitalize()}")
        plt.colorbar(scatter, label="True Redshift (z_cgal)")  # Add color bar
        plt.grid()

        # Save the plot
        plot_filename = f"{output_path}/color_color_diagram_{x_color}_vs_{y_color}_{subset_name}.png"
...
def plot_density_histogram(healpix_map, threshold, output_path):
    ...
    non_zero_values = healpix_map[healpix_map > 7000]
...
def main():
    ...
    healpix_map_valid = healpix_map[healpix_map > 7000]
   ...
   # Plot magnitude distributions for the i-band
    ...
    plot_color_color_diagram(data, output_paths["plots"], subset_name="all")
    plot_color_color_diagram(high_data, output_paths["plots"], subset_name="high_density")
    plot_color_color_diagram(low_data, output_paths["plots"], subset_name="low_density")
    plot_color_color_diagram(random_data, output_paths["plots"], subset_name="random")
